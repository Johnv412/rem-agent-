...
- [x] **[GATED] G-CI — DONE (2026-08-26).** Secret set via gh CLI; after
  John granted the `workflow` scope, both workflows were pushed and
  ci-staging/ removed. First runs on GitHub's runners: **CI ✅, CI Extra ✅
  (keyless E2E, dashboard honesty, wheel+twine), Live Gemini Canary ✅**
  ("CANARY PASSED: live Gemini consolidation + supersession verified").
  Nightly canary schedule active (07:17 UTC). One CI-only test flake
  fixed en route: newer mcp masks ToolError messages, so the assertion
  now pins the invariant (error, never success) instead of the
  version-dependent message text.
